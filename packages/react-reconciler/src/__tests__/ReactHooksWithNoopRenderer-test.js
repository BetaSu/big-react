...
describe('ReactHooksWithNoopRenderer', () => {
	...
	describe('useState', () => {
		...
		// 	function Counter() {
		// 		const [count, updateCount] = useState(0);
		// 		dispatcher = updateCount;
		// 		return <Text text={'Count: ' + count} />;
		// 	}
		// 	const root = ReactNoop.createRoot();
		// 	root.render(<Counter />);
		// 	expect(Scheduler).toFlushAndYield(['Count: 0']);
		// 	expect(root.getChildren()).toEqual([span('Count: 0')]);
		// 	await act(async () => dispatcher(1));
		// 	expect(Scheduler).toHaveYielded(['Count: 1']);
		// 	expect(root.getChildren()).toEqual([span('Count: 1')]);
		// 	await act(async () => dispatcher((count) => count + 10));
		// 	expect(Scheduler).toHaveYielded(['Count: 11']);
		// 	expect(root.getChildren()).toEqual([span('Count: 11')]);
		// });
